...
<body>
    ...
    <div class="contenedor contenedor-article">
        ...
        <div class="grid-two">
            ...
            $consulta = "SELECT id_producto, pro_nombre, pro_precio FROM Producto";
            $cn = conectarBD();
            $rsp = $cn->query($consulta);
            if ($rsp->num_rows > 0) {
                $i = 1;
                while ($fila = $rsp->fetch_assoc()) {
                    $pro_id = $fila["id_producto"];
                    $pro_name = $fila["pro_nombre"];
                    $pro_price = $fila["pro_precio"];
                    
                    echo ' 
                        <div class="producto">
                            <a href="producto.html" class="article-item">
                                <h3>Home elements</h3>
                                <img class="producto_image" src="./img/almacenado/'.$i.'.png" alt="imagen camisa">
                                <div class="producto_informacion">
                                    <p class="producto_name">'.$pro_name.'</p>
                                    <strong class="producto_description">Total Protection</strong>
                                    <p class="producto_descuento">'.$pro_price.'</p>
                                    <div class="ranking">
